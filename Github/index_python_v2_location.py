...
@@ -104,103 +161,114 @@ def send_message(to, text):
...
def validar_respuesta(indice, texto, estado=None, lat=None, lon=None):
    try:
        num = int(texto)
    except ValueError:
        num = None
...
def whatsapp_webhook():
    ...
    if estado['indice'] == 2:
        ...
        for idx, est in enumerate(lista_estados, 1):
            texto += f"{idx}) {est}\n"
        texto = f"P3. {estado['nombre']}, {preguntas_generales[2]}"
